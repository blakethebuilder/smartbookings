...
export default function Layout() {
  const { staff, isAdmin, isSuperadmin, switchClient, currentClient, logout } = useAuth()
  const navigate = useNavigate()
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const { branding } = useBranding()

  const navItems = isAdmin ? (
    isSuperadmin ? [...adminNavItems, { to: '/clients', icon: Building2, label: 'Clients' }] : adminNavItems
  ) : staffNavItems
      ...
      `}>
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {isAdmin ? `${branding.staff_role_admin} Portal` : `${branding.staff_role_worker} HQ`}
            </p>
          </div>
          ...
          <div className="px-4 py-3 border-b border-gray-200 flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 flex items-center gap-1">
                {isAdmin ? <Crown size={10} style={{ color: branding.primary_color }} /> : null}
                {isAdmin ? branding.staff_role_admin : branding.staff_role_worker}
              </p>
            </div>
          </div>
        )}
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 border-l-[3px] ${
                  isActive
                    ? 'border-current text-sb-orange bg-orange-50/70'
                    : 'border-transparent text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                }`
              }
              style={({ isActive }) => isActive ? { color: branding.primary_color, backgroundColor: branding.primary_color + '12', borderColor: branding.primary_color } : undefined}
            >
              <Icon size={18} />
              {displayLabel}
            </NavLink>
        ...
        <div className="p-4 border-t border-gray-200">
          <button
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-500 hover:text-sb-orange hover:bg-gray-50 w-full transition-colors"
          >
            <LogOut size={18} />
            Logout
          </button>
