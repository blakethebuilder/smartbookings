...
import { Plus, Trash2, Edit, Loader2, Building2, Globe } from 'lucide-react'
...
export default function Clients() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error('Failed to save client. Check if subdomain is unique.')
    } finally {
      setSaving(false)
    }
  }
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={() => setConfirmDelete(client)} className="p-1.5 rounded hover:bg-red-500/20 text-gray-500 hover:text-red-600" title="Delete">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <Badge variant={client.is_active ? 'success' : 'secondary'} className="uppercase">
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                ...
                <Input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full" />
              </div>
              <div>
                <label className="text-sm text-gray-600 mb-1 block">Subdomain *</label>
                <Input type="text" value={form.subdomain} onChange={e => setForm(f => ({ ...f, subdomain: e.target.value }))} className="w-full font-mono" />
              </div>
            </div>
            ...
            <div className="border-t border-gray-200 pt-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  ...
                  <Input type="text" value={form.resource_label} onChange={e => setForm(f => ({ ...f, resource_label: e.target.value }))} className="w-full" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 mb-1 block">Resource Label (Plural)</label>
                  <Input type="text" value={form.resource_label_plural} onChange={e => setForm(f => ({ ...f, resource_label_plural: e.target.value }))} className="w-full" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 mb-1 block">Admin Role Title</label>
                  <Input type="text" value={form.staff_role_admin} onChange={e => setForm(f => ({ ...f, staff_role_admin: e.target.value }))} className="w-full" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 mb-1 block">Worker Role Title</label>
                  <Input type="text" value={form.staff_role_worker} onChange={e => setForm(f => ({ ...f, staff_role_worker: e.target.value }))} className="w-full" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 mb-1 block">Booking Verb / CTA</label>
                  <Input type="text" value={form.booking_verb} onChange={e => setForm(f => ({ ...f, booking_verb: e.target.value }))} className="w-full" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input type="text" value={form.logo_url} onChange={e => setForm(f => ({ ...f, logo_url: e.target.value }))} className="w-full" placeholder="https://..." />
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-gray-200 pt-4">
              ...
              <div>
                ...
                <Input type="text" value={form.customer_fields} onChange={e => setForm(f => ({ ...f, customer_fields: e.target.value }))} className="w-full" />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={saving || !form.name || !form.subdomain}>
      ...
      <Dialog open={!!confirmDelete} onOpenChange={(open) => { if (!open) setConfirmDelete(null) }}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-gray-600">
            Delete "{confirmDelete?.name}"? This cannot be undone. All associated data will remain.
          </p>
          <DialogFooter>
